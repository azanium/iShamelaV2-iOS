...
+ (BOOL)unzipFileAtPath:(NSString *)path withFileName:(NSString *)fname toDestination:(NSString *)destination toFileName:(NSString *)destFileName error:(NSError **)error {
	// Begin opening
	zipFile zip = unzOpen((const char*)[path UTF8String]);	
	if (zip == NULL) {
		NSDictionary *userInfo = [NSDictionary dictionaryWithObject:@"failed to open zip file" forKey:NSLocalizedDescriptionKey];
	...
	do {
        ...
        if (strcmp(filename, [fname UTF8String]) != 0)
        {
			unzCloseCurrentFile(zip);
			ret = unzGoToNextFile(zip);
			continue;            
